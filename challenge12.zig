...
pub fn EncryptionOracle(comptime keysize: usize) type {
    return struct {
        ...
        fn createInput(self: *Self, plaintext: []const u8) ![]u8 {
            const padded_len = pad.calcWithPkcsSize(keysize/8, secret_prefix.len + plaintext.len);
            var secret_and_plaintext = try self.allocator.alloc(u8, padded_len);

            // create (prefix || plaintext)
            var idx: usize = 0;
            while (idx < secret_prefix.len) : (idx += 1) {
                secret_and_plaintext[idx] = secret_prefix[idx];
            }
            idx = 0;
            while (idx < plaintext.len) : (idx += 1) {
                secret_and_plaintext[idx + secret_prefix.len] = plaintext[idx];
            }

            pad.pkcsPad(keysize/8, secret_and_plaintext, secret_and_plaintext[0..secret_prefix.len + plaintext.len]);
